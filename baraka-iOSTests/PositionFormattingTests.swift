...
struct PositionFormattingTests {
    @Test func computesDisplays() {
        let instrument = InstrumentEntity(from: Instrument(ticker: "AAPL", name: "Apple", exchange: "NASDAQ", currency: "$", lastTradedPrice: 10))
        let position = PositionEntity(instrument: instrument, quantity: 2, averagePrice: 8, cost: 16, marketValue: 20, pnl: 4, pnlPercentage: 25)
